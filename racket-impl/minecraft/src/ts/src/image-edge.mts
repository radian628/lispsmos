...
export type ImageEdgesOutput = {
    type: "image-edges",
    uniqueColors: number[],
    vertexPositions: [number, number][][]
};

function getMaxIndex<T>(arr: T[], callback: (elem: T) => number) {
    let val = -Infinity;
    let index = -1;
    arr.forEach((e, i) => {
...
function joinStraightLines(polyline: [number, number][]) {
    let newPolyline: [number, number][] = [];
    let prevAngle = -9999999;
    for (const segment of polyline) {
        let angle = Math.atan2(
            (newPolyline[newPolyline.length - 1] ?? [-1, -1])[1] - segment[1], 
            (newPolyline[newPolyline.length - 1] ?? [-1, -1])[0] - segment[0]
        );
...
export function findImageEdges(input: ImageEdgesInput): ImageEdgesOutput {
    const uniqueColors = new Set<number>();
    input.colors.forEach(col => uniqueColors.add(col));

    const vertexPositionLists: [number, number][][] = [];

    uniqueColors.forEach(uniqueCol => {

        const edgeHash: number[][] = [];
        const edgeList: [number, number][] = [];
        const edgesChecked: boolean[] = [];
        ...
        input.colors.forEach((col, i) => {
            ...
            function testSquareBoundary(compare: number, endpoint1: number, endpoint2: number) {
                if (col != compare) {
