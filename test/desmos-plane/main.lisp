...
(folder ((title "Buffers and Drawing"))
  ...
  (evalMacro addTerrainSlotProperties "
    ...
    let result = terrains.map((terrain, i) => {
      return [['=', 'terrainSlot'+index+'ToLoad', (i+1).toString()], ['PLYGet', terrain, (tspName == 'vertexunderscoreindices') ? 'face' : 'vertex', tspName]]
    });
    console.log(result);
    return result;
  ")
  ...
  (defineFindAndReplace createTerrainSlot index terrains
    (
      ...
      (= (concatTokens xTerrainSlot index) (piecewise 
        (addTerrainSlotProperties x terrains index)
        ((list))
      ))
      (= (concatTokens yTerrainSlot index) (piecewise 
        (addTerrainSlotProperties y terrains index)
        ((list))
      ))
      (= (concatTokens zTerrainSlot index) (piecewise 
        (addTerrainSlotProperties z terrains index)
        ((list))
      ))
      (= (concatTokens rTerrainSlot index) (piecewise 
        (addTerrainSlotProperties red terrains index)
        ((list))
      ))
      (= (concatTokens gTerrainSlot index) (piecewise 
        (addTerrainSlotProperties green terrains index)
        ((list))
      ))
      (= (concatTokens bTerrainSlot index) (piecewise 
        (addTerrainSlotProperties blue terrains index)
        ((list))
      ))
      (= (concatTokens iTerrainSlot index) (piecewise 
        (addTerrainSlotProperties vertexunderscoreindices terrains index)
        ((list))
      ))
    )
  )
  ...
  (= SceneWindingOrders (getCullingInformation vboProjected (z3 VBOCameraSpace) IndexBuffer ScenePolygonOrdering 18))
  ...
  (fn doPlanePhysicsStep deltaTime (
    piecewise
      ((== crashed 0)(,
        ...
        (piecewise ((== (mod globalTime 10) 0) (-> planePosForTerrainSlots planePos)))
      ))
    )
  )
